...
export class VectorStore {
  ...
  constructor(dbPath: string = "./data/github-docs.db") {
  ...
  async addDocument(doc: DocumentChunk): Promise<number> {
  ...
  async searchSimilar(queryEmbedding: number[], limit: number = 10, repoFilter?: string[]): Promise<SearchResult[]> {
  ...
  async getDocumentsByRepo(repo: string): Promise<DocumentChunk[]> {
  ...
  async getRepoStats(): Promise<{ repo: string; docCount: number; lastUpdated: string }[]> {
    const stmt = this.db.prepare(`
      SELECT 
        repo,
        COUNT(*) as doc_count,
