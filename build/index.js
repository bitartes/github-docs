...
server.tool("list_org_repos", "List all repositories for a GitHub organization", {
    ...
    try {
        ...
        const repoList = filteredRepos.map(repo => `- **${repo.name}** (${repo.fullName})\n  ${repo.description || 'No description'}\n  Last updated: ${new Date(repo.updatedAt).toLocaleDateString()}`).join('\n\n');
        return {
            content: [
                {
                    type: "text",
...
server.tool("index_repo_docs", "Index documentation from a GitHub repository into the vector database", {
...
server.tool("search_docs", "Search across indexed documentation using semantic similarity", {
...
server.tool("get_repo_stats", "Get statistics about indexed repositories", {}, async () => {
    try {
        ...
        const repoList = stats.map(stat => `- **${stat.repo}**: ${stat.docCount} documents (last updated: ${new Date(stat.lastUpdated).toLocaleDateString()})`).join('\n');
        return {
            content: [
                {
                    type: "text",
...
server.tool("find_dependencies", "Find potential dependencies and relationships between services based on documentation", {
...
server.tool("analyze_feature_impact", "Analyze potential impact of a feature change across services and dependencies", {
    ...
    try {
        ...
        if (changeType === "remove" || changeType === "deprecate") {
            if (uniqueDependencies.length > 0) {
                alerts.push(`🚨 **BREAKING CHANGE ALERT**: ${uniqueDependencies.length} services may be affected by ${changeType === "remove" ? "removing" : "deprecating"} "${feature}"`);
                uniqueDependencies.slice(0, 5).forEach((dep, index) => {
                    alerts.push(`${index + 1}. **${dep.document.repo}** (${(dep.similarity * 100).toFixed(1)}% match)\n   File: ${dep.document.filePath}\n   ${dep.document.content.substring(0, 150)}...`);
                });
            }
        }
...
server.tool("get_dev_recommendations", "Get development recommendations and best practices for a specific service or feature", {
    context: z.string().describe("Development context (service name, feature, or technology)"),
    type: z.enum(["best-practices", "patterns", "security", "performance", "testing"]).optional().describe("Type of recommendations to focus on"),
}, async ({ context, type }) => {
    try {
...
server.tool("check_breaking_changes", "Check for potential breaking changes when modifying APIs or services", {
    ...
    try {
        const breakingChangeIndicators = [
            ...
            "version", "migration", "upgrade", "contract", "interface"
        ];
